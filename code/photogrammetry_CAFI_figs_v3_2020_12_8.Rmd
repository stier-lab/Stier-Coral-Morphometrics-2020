...
fig3_raw <- plot_grid(invert_field, invert_hull, invert_pg, richness_field, richness_hull, richness_pg, nrow = 2, ncol = 3, align = "vh", labels = "AUTO")
...
# fig3_fin <- annotate_figure(fig3_raw,
...
photo_hull_trim <- december_photo %>% select(coral_id, volume = max_hull_volume, volume_aug = vol_hull_aug, growth = hull_growth, prop_growth = prop_hull_growth) %>% mutate(method = "Hull")
...
#     geom_point(size = 3) +
#     scale_color_manual(labels = c("Ellipse", "Hull", "Photo Model"), values = c("#edae52", "coral", "#9fb059")) +
...
joined_growth_v_vol <- ggplot(december_stacked, aes(volume_aug, growth, color = method)) +
  geom_smooth(method = "lm", formula = "formula1", fill = "grey92") +
  geom_point(size = 2) +
  stat_poly_eq(aes(label = paste(..rr.label.., sep = "~~~")), 
               label.x.npc = "right", label.y.npc = "bottom",
               formula = formula1, parse = TRUE, size = 5) +
...
# Try filtering out outlying coral

#december_stacked_trim <- december_stacked %>% filter(coral_id != "FE-POC37")

#joined_growth_v_vol_trim <- ggplot(december_stacked_trim, aes(volume_aug, growth, color = method)) +
#  geom_smooth(method = "lm", formula = "formula1", fill = "grey92") +
#  geom_point(size = 2) +
#  stat_poly_eq(aes(label = paste(..rr.label.., sep = "~~~")), 
#               label.x.npc = "right", label.y.npc = "bottom",
...
reg_sum <- function(input2, lm_x) {
  ...
  temp$"p" <- signif(summary(lm_x)$coefficients[2,4],3)
...
```{r cafi_morph_tab, include = FALSE, echo = FALSE}
# cafi_morph_tab1
# cafi_morph_tab2
# cafi_morph_tab3
# cafi_morph_tab4
...
richness_tab <- rbind(reg_sum("Ellipse", lm_richness_field), reg_sum("Convex Hull", lm_richness_hull),
                  reg_sum("Skeletal Volume", lm_richness_pg), reg_sum("Surface Area", lm_invert_sa),
                  reg_sum("Convexity", lm_conv), reg_sum("Interstitial Space", lm_interstitial))
...
tbl_1 <- kbl(tbl_raw, booktabs = TRUE) %>% 
  kable_classic() %>% 
