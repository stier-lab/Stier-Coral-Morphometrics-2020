...
# Plot currently is all one color - change to right colors if that's the final version
...
invert_field <- ggplot(cafi_coral, aes(x=volume_field, y=num_cafi))+ 
  geom_smooth(method=lm, formula = formula1, se = TRUE, color = "#edae52", fill = "#edae52")+
...
invert_pg <- ggplot(cafi_coral, aes(x=volume_pg, y=num_cafi))+ 
  geom_smooth(method=lm, formula = formula1, se = TRUE, color = "#5fb2b2", fill = "#5fb2b2")+
...
invert_hull <- ggplot(cafi_coral, aes(x=max_hull_volume, y=num_cafi))+ 
   geom_smooth(method=lm, formula = formula1, se = TRUE, color = "dodgerblue1", fill = "dodgerblue1")+
...
options(scipen = 999)
reg_sum <- function(input2, lm_x) {
  temp <- data.frame(matrix(ncol = 7))
  temp_cols <- c("Variable", "Intercept", "Slope", "95% CI", "F", "p", "r2")
  colnames(temp) <- temp_cols
  temp$Variable <- paste(input2)
  temp$Intercept <- signif(lm_x$coefficients[1], 3)
  temp$Slope <- signif(lm_x$coefficients[2], 3)
  temp$`95% CI` <- paste(eval(signif(confint(lm_x)[2,1],3)), "-", eval(signif(confint(lm_x)[2,2],3)))
  temp$"F" <- signif(summary(lm_x)$fstatistic[1],3)
  temp$"p" <- round(summary(lm_x)$coefficients[2,4],3)
  temp$r2 <- signif(summary(lm_x)$r.squared,3)
  temp
}

abun_tab <- rbind(reg_sum("Ellipse", lm_invert_field), reg_sum("Convex Hull", lm_invert_hull),
                  reg_sum("Skeletal Volume", lm_invert_pg), reg_sum("Surface Area", lm_invert_sa),
                  reg_sum("Convexity", lm_conv), reg_sum("Interstitial Space", lm_interstitial))
...
lm_conv_rich <- lm(cafi_richness~convexity, data = cafi_coral)

lm_interstitial_rich <- lm(cafi_richness~interstitial_space, data = cafi_coral)

lm_invert_sa_rich <- lm(cafi_richness~SA, data = cafi_coral)


richness_tab <- rbind(reg_sum("Ellipse", lm_richness_field), reg_sum("Convex Hull", lm_richness_hull),
                  reg_sum("Skeletal Volume", lm_richness_pg), reg_sum("Surface Area", lm_invert_sa_rich),
                  reg_sum("Convexity", lm_conv_rich), reg_sum("Interstitial Space", lm_interstitial_rich))
...
tbl_raw <- tbl_raw %>% 
  rename("$\\beta$" = Slope, "r$^2$" = r2) %>% 
  mutate_if(is.numeric, as.character) %>% 
  mutate(p = if_else(as.numeric(tbl_raw$p)>0.001, as.character(tbl_raw$p), "<0.001")) %>% 
  mutate_at("r$^2$", as.numeric)


tbl_1 <- kbl(tbl_raw, booktabs = TRUE, digits = c(1,1,1,1,1,1,3)) %>% 
  kable_classic() %>% 
  kable_styling(bootstrap_options = c("striped", "bordered")) %>% 
  pack_rows("Abundance", 1,6) %>% 
  pack_rows("Richness", 7,12) %>% 
  column_spec(6, bold = ifelse(tbl_raw$p < 0.05, "TRUE", "FALSE"))
```
...
```{r residual_plot_prep, include = FALSE}
cafi_coral<- cafi_coral %>% mutate(ellipse_abun_resid = residuals(lm_invert_field))

convex_resid <- ggplot(cafi_coral, aes(x=convexity, y = ellipse_abun_resid))+ 
  geom_smooth(method=lm, formula = formula1, fill = "lightblue", color = "black")+
  geom_point(pch = 21, size = 4, alpha = 0.7, color = "black", fill = "coral")+
  theme_classic()+
  labs(y=expression(atop("Residual", paste("(Ellipsoid volume vs CAFI abundance)"))), x= expression(paste("Convexity"))) +
  stat_poly_eq(aes(label = paste(..rr.label.., sep = "~~~")), 
               label.x.npc = "right", label.y.npc = "bottom",
               formula = formula1, parse = TRUE, size = 5) +
  theme(text = element_text(size = 13))
...
# ggsave(here("code","figures","residual_plot.pdf"), convex_resid, device = "pdf", useDingbats = FALSE, height = 4, width = 6)

lm_resid <- lm(ellipse_abun_resid~convexity, data = cafi_coral)
summary(lm_resid)
```