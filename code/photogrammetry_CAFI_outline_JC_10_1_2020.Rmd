...
``` {r growth_boxplots, include = TRUE, echo = FALSE, message = FALSE}

manual_growth_box <- ggplot(december_manual, aes(x = "Ellipse", y = vol_growth_man)) +
  geom_boxplot(width = 0.5, outlier.shape = NA) + 
  geom_jitter(width = 0.05, size = 2, alpha = 0.5, color = "coral") +
  theme_classic() +
  xlab("") +
  ylab(expression(paste("Growth (cm"^{3},")"))) +
  ylim(min(december_manual$vol_growth_man) - 100, max(december_manual$vol_growth_man) +100) +
  theme(axis.text.x = element_text(size = 15, face = "bold"), axis.ticks.x = element_blank(), axis.line.x = element_blank())

photo_growth_box <- ggplot(december_photo, aes(x = "Photo", y = vol_growth_pg)) +
  geom_boxplot(width = 0.5, outlier.shape = NA) + 
  geom_jitter(width = 0.05, size = 2, alpha = 0.5, color = "coral") +
  theme_classic() +
  labs(x = "", y = "") +
  ylim(min(december_manual$vol_growth_man) - 100, max(december_manual$vol_growth_man) +100) +
  theme(axis.text.x = element_text(size = 15, face = "bold"), axis.ticks = element_blank(), axis.line = element_blank(),
        axis.text.y = element_blank())

manual_growth_prop_box <- ggplot(december_manual, aes(x = "Ellipse", y = prop_growth_man)) +
  geom_boxplot(width = 0.5, outlier.shape = NA) + 
  geom_jitter(width = 0.05, size = 2, alpha = 0.5, color = "coral") +
  theme_classic() +
  xlab("") +
  ylab("% Growth") +
  ylim(min(december_manual$prop_growth_man) - 100, max(december_manual$prop_growth_man) +100) +
  theme(axis.text.x = element_text(size = 15, face = "bold"), axis.ticks.x = element_blank(), axis.line.x = element_blank())

photo_growth_prop_box <- ggplot(december_photo, aes(x = "Photo", y = prop_growth_pg)) +
  geom_boxplot(width = 0.5, outlier.shape = NA) + 
  geom_jitter(width = 0.05, size = 2, alpha = 0.5, color = "coral") +
  theme_classic() +
  labs(x = "", y = "") +
...
growth_comb_box <- plot_grid(manual_growth_box, photo_growth_box, align = 'vh', labels = NA)
prop_growth_comb_box <- plot_grid(manual_growth_prop_box, photo_growth_prop_box, align = 'vh', labels = NA)
