...
* Traditionally, these have been made using cheap, readily accessible tools such as observer-based surveys and manual methods
...
It appears that the estimates of ellipsoid volume overestimate actual coral volume, but correspond well to convex hull volume as well as other manual measurements. There also appears to be a slight offset between manual and digital measurements of height, with manual measurements consistently a small amount larger than digital measurements. Larger corals also may experience a higher degree of overestimation from calculations of ellipsoid volume vs. coral volume.

Let's look at the absolute and proportional differences between coral, convex hull, and ellipsoid volumes across the span of coral sizes to see whether the sizes of certain corals may be more closely approximated via manual measurements
...
Next, we will look at how the relative amounts of variation in ecological phenomena may be explained by the improved resolution offered by digital measurements. In this case, we look at the relationship between the composition of resident associated fishes and invertebrates (CAFI) and manually vs. digitally derived measurements of coral volume and available living space.
...
richness_pg <- ggplot(cafi_coral, aes(x=volume_pg, y=cafi_richness))+ 
  ...
  geom_smooth(method=lm)+
  theme_classic()+
  labs(y="CAFI Richness", x= expression(paste("Hull Volume (cm"^{3},")")))
...
Now that we've seen that photogrammetry-enabled measurements could potentially explain more variation in CAFI community composition  than manual measurements, let's look at which attributes that can be measured with photogrammetry best predict CAFI abundance and richness
...
Coral volume and surface area explain the highest amount of variation in CAFI abundance of all of the available metrics of coral morphology. The same is true for CAFI species richness, although digitally measured height vs. CAFI richness explains an equal amount of variation to volume. Since height is also a metric we have measured manually, let's quickly compare height vs. CAFI metrics measured manually and digitally.

```{r height prep, include = TRUE, echo = FALSE, messages = FALSE}

lm_manual_height_abun <- lm(num_cafi~height_field, data = cafi_coral)
lm_manual_height_rich <- lm(cafi_richness~height_field, data = cafi_coral)
lm_height_abun <- lm(num_cafi~height_pg, data = cafi_coral)

height

manual_height_abun <- ggplot(cafi_coral, aes(x=height_field, y=num_cafi))+ 
  geom_point()+
  geom_smooth(method=lm)+
  theme_classic()+
  xlab("Height (cm)") +
...
manual_height_rich <- ggplot(cafi_coral, aes(x=height_field, y=cafi_richness))+ 
  geom_point()+
  geom_smooth(method=lm)+
  theme_classic()+
  xlab("Height (cm)") +
...
cafi_tab_height_man <- tab_model(lm_manual_height_abun, lm_manual_height_rich,
          show.intercept = FALSE,
          pred.labels = c(pl_height, pl_height),
          string.est = "Slope",
          string.ci = "Slope CI (95%)",
          string.p = "p-Value",
          digits=5,
          dv.labels = c("Manual Height vs. Abundance", "Manual Height vs. Richness"))

cafi_tab_height_digi <- tab_model(lm_height_abun, lm_height,
          show.intercept = FALSE,
          pred.labels = c(pl_height, pl_height),
          string.est = "Slope",
          string.ci = "Slope CI (95%)",
          string.p = "p-Value",
          digits=5,
          dv.labels = c("Photogrammetry Height vs. Abundance", "Photogrammetry Height vs. Richness"))

height_abun <- ggplot(cafi_coral, aes(x=height_pg, y=num_cafi))+ 
  geom_point()+
  geom_smooth(method=lm)+
  theme_classic()+
  xlab("Photogrammetry Height (cm)") +
  ylab("CAFI Abundance")


height_rich <- ggplot(cafi_coral, aes(x=height_pg, y=cafi_richness))+ 
  geom_point()+
  geom_smooth(method=lm)+
  theme_classic()+
  xlab("Photogrammetry Height (cm)") +
...
height_fig_title <- ggdraw() + draw_label("Height Measurements vs. CAFI", fontface = "bold", size = 14)
height_fig_raw <- plot_grid(height_abun, height_rich, manual_height_abun, manual_height_rich, nrow = 2, ncol = 2, align = 'vh')

height_fig <- plot_grid(height_fig_title, height_fig_raw, nrow = 2, rel_heights = c(.1,1))

annotate_figure(height_fig,
                fig.lab = "Figure 5", fig.lab.face = "bold")

cafi_tab_height_digi
cafi_tab_height_man


```

Although height measured manually does seem to capture a substantial portion of the variation in CAFI abundance and richness, the digital measurements still correspond more closely to CAFI community characteristics. However, this improved resolution may not be ecologically or analytically meaningful

# Preliminary interpretations

The broad conclusions I would draw from these preliminary analyes and visualizations are:

* Photogrammetry, even with "point and shoot" cameras, appeared to characterize coral colonies well, tracking manual measurements of length, width, and height closely. These close correlations also lend credibility to volume, surface area, and other more advanced measurements of coral morphology made using digital models

* Ellipsoid calculations of coral volume are a reasonable approximation of the maximum amount of space a coral colony occupies, as it seems to compare closely with convex hull volume. However, these approximations overestimate coral volume by up to a factor of 5, depending somewhat on coral size and morphology (higher amount of overestimation in widely branched, large corals)

* Advanced measurements made using photogrammetry of coral morphology, particularly those of coral skeletal volume, may improve ecological predictions based on substantially higher correlations with CAFI characteristics than hull volume or ellipspoid volume. In this case, height also might be a reasonable proxy measurement that could predict some CAFI community characteristics, whether measured manually or digitally

* Close examination of how these advanced morphological descriptions of living space within a coral relate to CAFI community characteristics in different ways are likely to yield more advanced ecological insight than available through analyses of relationships to linear measurements alone (those available manually)

* Whether or not photogrammetry is a worthwhile exercise will depend heavily on the study system, question, and level of resolution needed to answer project-specific questions. However, photogrammetry has the potential to improve predictive models that rely on morphological measurements, and to lead to new, more complex and subtle ways of thinking about how the morphology of foundation species determines ecological phenomena.

# Next steps

* Growth spiderweb figure
* PCA of coral morphometrics vs. CAFI characteristics
* Finish QA/QC - 6 models didn't come out well in first pass, worth second attempt?
* Make convex hull measurements for "control" corals (~30, won't take long)
