...
cor_var2 <- cafi_coral %>%
  dplyr::select(sqrt_num_cafi,sqrt_volume_pg,sqrt_volume_field,sqrt_SA,sqrt_avg_w,sqrt_interstitial_space,sqrt_sphericity,convexity,packing)
...
height <- ggplot(dim_bind, aes(x=height_pg, y=height_field))+ 
  geom_point()+
  geom_smooth(method=lm, se=TRUE)+
  scale_y_continuous(breaks=seq(0,max(dim_bind$height_field), by = 5000))+
  scale_x_continuous(breaks=seq(0,max(dim_bind$height_pg), by = 2000))+
  theme_bw()+
...
length <- ggplot(dim_bind, aes(x=length_pg, y=length_field))+ 
  geom_point()+
  geom_smooth(method=lm, se=TRUE)+
  scale_y_continuous(breaks=seq(0,max(dim_bind$length_field), by = 5000))+
  scale_x_continuous(breaks=seq(0,max(dim_bind$length_pg), by = 2000))+
  theme_bw()+
...
width <- ggplot(dim_bind, aes(x=width_pg, y=width_field))+ 
  geom_point()+
  geom_smooth(method=lm, se=TRUE)+
  scale_y_continuous(breaks=seq(0,max(dim_bind$width_field), by = 5000))+
  scale_x_continuous(breaks=seq(0,max(dim_bind$width_pg), by = 2000))+
  theme_bw()+
  labs(y= "Manual w cm", x= "Software w cm")+
  geom_abline(intercept = 0, slope = 1, color='blue')
  
fig2 <- ggarrange(volume, height, length, width,
          ncol = 2, nrow = 2)
...
The below graphs compare the squareroot transformed software estimated volume, height, length and width measurements to the same coral's squareroot transformed manual measurements. A one to one line was fit to show how differences in measuremets vary with increasing coral size.
