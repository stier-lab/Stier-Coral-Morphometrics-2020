...
coral_dim <- coral_dim %>% 
  dplyr::select(coral_id, branch, cafi, size_class, volume_field, volume_pg, SA, height_pg, avg_w_cm, interstitial_space, SAV, convexity, packing, sphericity) %>%
  filter(cafi=="empty")
...
cafi_coral <- merge(cafi_summarized2, coral_dim, by ="coral_id") %>%
  ...
  dplyr::select(coral_id, branch, cafi, size_class, volume_field, volume_pg, SA, height_pg, avg_w_cm, interstitial_space, SAV, convexity, packing, sphericity, num_cafi, cafi_present, cafi_richness, sw) 
...
dim_bind <- merge(dim_pg, dim_field, by="coral_id") %>%
  ...
  summarise(coral_id,volume_pg,volume_field,
            ...
            prop_est=volume_pg/volume_field, #proportion overestimation of elipsoide compared to software volume
...
```{r, message=FALSE}
#Volume
#summary(lm(formula = volume_pg ~ volume_field, data = dim_bind))
volume <- ggplot(dim_bind, aes(x=volume_pg, y=volume_field))+ 
  ...
  geom_abline(intercept = 611.4145, slope = 1, color='black') #x-intercept pulled from summary data
...
plot_est <- ggplot(dim_bind, aes(x=volume_pg, y=diff_est))+ 
  geom_point()+
  geom_smooth(method=lm, se=TRUE)+
  theme_bw()+
   labs(y= expression(paste("elipsoid overestim. cm"^{3})), x= expression(paste("software V cm"^{3})))

lm_plot_est <- lm(diff_est~volume_pg, data=dim_bind)
...
tab_model(lm_plot_est,
          pred.labels = pl,
          dv.labels = "Overestim. difference")
```

```{r}
#Proportional overestimation
plot_prop <- ggplot(dim_bind, aes(x=volume_pg, y=prop_est))+
  geom_point()+
  geom_smooth(method=lm, se=TRUE)+
  theme_bw()+
   labs(y= "Proportion over-estim.", x= expression(paste("software V cm"^{3})))

lm_plot_prop <- lm(prop_est~volume_pg, data=dim_bind)
...
tab_model(lm_plot_prop,
          pred.labels = pl,
          dv.labels = "Proportion over-estim.")

fig2 <- ggarrange(plot_est, plot_prop,
          ncol = 2, nrow = 1)
```
...
#Check qqplots for normality and run Shapiro Wilk on residuals
lm_invert_field <- lm(num_cafi~volume_field, data=cafi_coral)
par(mfrow=c(2,2))
plot(lm_invert_field)
par(mfrow=c(1,1))

res_invert_field=lm_invert_field$residuals
shapiro.test(res_invert_field)

invert_field
```

With a $p=0.131$ we fail to reject the null hypothesis that the residuals are normally distributed. Even though the data is normally distributed, linearity and homoescedascity assumptions are not met. We will try a Box-Cox transformation.

```{r, include=TRUE}
bc1 = boxcox(lm_invert_field)
best.lam1 = bc1$x[which(bc1$y==max(bc1$y))]
fullmodel.inv1 = lm((num_cafi)^0.14 ~ volume_field, data=cafi_coral)

par(mfrow=c(2,2))
plot(fullmodel.inv1)
par(mfrow=c(1,1))
```
Linearity and homoscedacity are not met using this transformation suggesting that a stronger log transformation is required.
...
res_invert_pg=lm_invert_pg$residuals
shapiro.test(res_invert_pg)
```

With a $p-value=0.5821$ we fail to reject the null hypothesis that the residuals of our non-transformed data are normally distributed. Linearity and homoescedascity assumptions are not met. We will still try a Box-Cox transformation.

```{r}
bc2=boxcox(lm_invert_pg)
best.lam2=bc2$x[which(bc2$y==max(bc2$y))]
fullmodel.inv2 = lm((num_cafi)^0.22 ~ volume_pg, data=cafi_coral)
...
After our transformation is applied, linearity, homoscedacity and normality assumptions are met. 

```{r, message=FALSE}
fig5 <- ggarrange(invert_field, invert_pg, #sqrt_invert_field, sqrt_invert_pg,
          ncol = 2, nrow = 2)
...
pl <- c(
  '(Intercept)' = "Intercept",
  volume_field = "Elispoid V"
)
tab_model(fullmodel.inv1,
          pred.labels = pl,
          dv.labels = "Box-Cox invert abundance")
...
tab_model(lm_invert_pg,
          pred.labels = pl,
          dv.labels = "Invert abundance")

#HTML table from regression results
pl <- c(
  '(Intercept)' = "Intercept",
  volume_pg = "Software est. V"
)
tab_model(fullmodel.inv2,
          pred.labels = pl,
          dv.labels = "Box-Cox invert abundance")
```

Based on the above results from the table, we can explain 13.7% more variance in the non-transformed data based on the $R^2$ values. 

Based on the above results from the table, we can explain 10.4% more variance in the sqrt transformed data based on the $R^2$ values. 

More variance could be explained for the non-transformed data. Although the residuals were normal for both the elipsoid and software estimated volumes, the linearity and homoscedascity assumptions were not met. 

```{r}
#Fit poisson regression 
glm1 <- glm(num_cafi~volume_pg, family=quasipoisson, data=cafi_coral) #quasipoisson used to correct for standard errors
summary(glm1)

g_pg<-ggplot(cafi_coral,aes(x=volume_pg,y=num_cafi))+
  geom_point()+
...
g_field<-ggplot(cafi_coral,aes(x=volume_field,y=num_cafi))+
  geom_point()+
  geom_smooth(method="glm", family="quasipoisson", se=TRUE)+
  ggtitle("Quasipoisson")
