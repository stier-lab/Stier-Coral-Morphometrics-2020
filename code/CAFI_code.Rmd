...
coral_dim$diff_est <- coral_dim$volume_field-coral_dim$volume_pg

coral_dim$prop_est <- (coral_dim$volume_field - coral_dim$volume_pg)/coral_dim$volume_pg
...
#plot2 <- ggplot(cafi_coral, aes(x=volume_field, y=num_cafi))+ 
  #geom_point()+
  #geom_smooth(method=lm, se=TRUE)+
  #theme_classic()+
  #theme(axis.title.y=element_blank(),
...
We will now fit a poisson general linear model to our invertebrate count and richness data 
...
With $p=0.108$ and $p=0.233$, we fail to reject the null hypothesis that the two models comparing elipsoid and software volume are indistinguishable for both abundance and richness using the Vuong test comparing non-nested models. Theoretically, the lower AIC values for software estimated volume estimating log-likelihood as shown in the tables reflect a relatively better quality model to fit the data. 