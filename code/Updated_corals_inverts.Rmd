...
```{r, include=FALSE}
#Combine cleaned cafi data with coral dimensions
...
vol_branch <- ggplot(cafi_coral2, aes(x=volume_pg, y= avg_w_mm, shape= branch, color= branch, linetype= branch))+ #changed 'group' to 'shape'
  geom_point()+
...
fit_vol_branch_tight <- lm(avg_w_mm~volume_pg, data=cafi_coral2, subset=(branch=="tight"))
```
...
fit_vol_branch_wide <- lm(avg_w_mm~volume_pg, data=cafi_coral2, subset=(branch=="wide"))
```
...
fit_vol_SA_tight <- lm(avg_w_mm~volume_pg, data=cafi_coral2, subset=(branch=="tight"))
```
...
invert_branch <- ggplot(cafi_coral2, aes(x=avg_w_mm, y=num_cafi, shape=branch, color=branch, linetype=branch))+ #color=as.factor(branch)))+
  geom_point()+
  geom_smooth(method = lm, se=TRUE, aes(fill=branch))+ 
  theme_classic()+
  #facet_wrap(~branch)+
...
fit_invert_branch_tight <- lm(num_cafi~avg_w_mm, data=cafi_coral2, subset=(branch=="tight"))
```
...
fit_invert_branch_wide <- lm(num_cafi~avg_w_mm, data=cafi_coral2, subset=(branch=="wide"))
```
